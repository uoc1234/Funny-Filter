...
import com.titanbbl.funny.face.filter.game.databinding.ActivityTimeDelayGridBinding
...
class TimeDelayGridActivity : BaseActivity<ActivityTimeDelayGridBinding>() {
    ...
    override fun getLayoutActivity(): Int = R.layout.activity_time_delay_grid
    ...
    private fun setupClickListeners() {
        ...
        mBinding.tvNameMusic.click {
            if (isAutoCaptureRunning) {
                Toast.makeText(this, "Cannot change music while recording", Toast.LENGTH_SHORT)
                    .show()
            } else {
    ...
    private fun disableMusicSelectionButtons() {
        try {
            mBinding.tvNameMusic.isClickable = false
            mBinding.tvNameMusic.isFocusable = false
            mBinding.tvNameMusic.alpha = 0.5f
            
            mBinding.ivClose.isClickable = false
            mBinding.ivClose.isFocusable = false
            mBinding.ivClose.alpha = 0.5f
            
            // Vô hiệu hóa toàn bộ top bar chứa nhạc
            mBinding.flTopBar.isClickable = false
            mBinding.flTopBar.isFocusable = false
            mBinding.flTopBar.alpha = 0.5f
        } catch (e: Exception) {
            e.printStackTrace()
        }
    }
    ...
    private fun enableMusicSelectionButtons() {
        try {
            mBinding.tvNameMusic.isClickable = true
            mBinding.tvNameMusic.isFocusable = true
            mBinding.tvNameMusic.alpha = 1.0f
            
            mBinding.ivClose.isClickable = true
            mBinding.ivClose.isFocusable = true
            mBinding.ivClose.alpha = 1.0f
            
            // Kích hoạt lại toàn bộ top bar chứa nhạc
            mBinding.flTopBar.isClickable = true
            mBinding.flTopBar.isFocusable = true
            mBinding.flTopBar.alpha = 1.0f
        } catch (e: Exception) {
            e.printStackTrace()
        }
    }
        ...
        totalRecordingTimer = object : CountDownTimer(totalDurationMs, 1000) {
            ...
            override fun onFinish() {
                if (isAutoCaptureRunning) {
                    isCapturingCompleted = true
                    stopAutoCapture()
                    Toast.makeText(this@TimeDelayGridActivity,
