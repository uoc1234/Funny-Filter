...
class HandDetectViewModel : ViewModel() {
    ...
    private val _isHandOpen = MutableLiveData<Boolean>()
    val isHandOpen: LiveData<Boolean> = _isHandOpen
    
    // For tracking hand position stability
    private var lastUpdateTime = 0L
    private val updateThreshold = 16L // ~60fps
    private var lastX = 0f
    private var lastY = 0f
    private val minMoveDistance = 2f // Minimum movement to update for hand tracking
    
    init {
        _drawingMode.value = false
        _isDrawing.value = false
        _isHandOpen.value = true
    }

    fun updateHandPosition(x: Float, y: Float, isHandOpen: Boolean) {
