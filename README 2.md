#### Template android project by BinaryBridgeLabs
#### How to use
### Screen
...
```
    BBLTrackingHelper.logEvent(AnalyticsHelper.ClickEditButton, Bundle?)
```
...
private fun addTrackingMoveScreen(fromContext: Activity, intent: Intent) {
        intent.component?.className?.substringAfterLast(".")?.let {
            BBLTrackingHelper.fromScreenToScreen(
